...
>>>>>>> 5c6c7892ef950b17bd809fc07e33a36627ed86d1
}
...
<<<<<<< HEAD
    ...
    return 0;
=======
    for (i = 0;i <= superb.posUltimoBloqueMB; i++){
		escribir_bit(i,1);
	}
    return 0;           
>>>>>>> 5c6c7892ef950b17bd809fc07e33a36627ed86d1
    ...
    for (i = SB.posPrimerBloqueAI; i <= SB.posUltimoBloqueAI; i++){
		for (j = 0; j < (BLOCKSIZE / sizeof(struct inodo)); j++){
			...
			if (variable_incremental < SB.totInodos) {
                ...
                // Ultimo de la lista apunta a un numero muy grande (NULL)
				inod[j].punterosDirectos[0] = UINT_MAX;
			}
>>>>>>> 5c6c7892ef950b17bd809fc07e33a36627ed86d1
		}
...
unsigned char leer_bit (unsigned int nbloque){
    ...
    mascara >>= posbit; // Desplazamiento de bits a la derecha
    mascara &= buffeMB[nbyte]; // Operador AND para bits
    mascara >>= (7 - posbit); // Desplazamiento de bits a la derecha
    return mascara;
}
...
int reservar_inodo(unsigned char tipo, unsigned char permisos){ 
    ...
    // Si hay algun inodo libre, reservamos uno
    if (SB.cantInodosLibres > 0){
        struct inodo inodAux = leer_inodo (SB.posPrimerInodoLibre);
        // Inicializamos el inodo
        struct inodo inodo;
        inodo.tipo = tipo;
	    inodo.permisos = permisos;
        ...
        // Escribe el inodo reservado
        if (escribir_inodo(inodo,SB.posPrimerInodoLibre) == -1) return -1;
        // Hacemos que el inodo auxiliar(inodAux) sea el primer inodo libre.
        int posInodoReservado = SB.posPrimerInodoLibre;
	    // Apuntamos al siguiente inodo libre
        SB.posPrimerInodoLibre = inodAux.punterosDirectos[0]; 
	    // Decrementamos el numero de inodos libres
        SB.cantInodosLibres--;
	    // Actualizamos el superbloque
        if (bwrite(0, &SB) == -1) return -1;
